...
const GradientBlinds: React.FC<GradientBlindsProps> = ({
  ...
  useEffect(() => {
    ...
    // IMPORTANT: do not block clicks on your hero content
...
`;
    ...
    // Pointer tracking (window listener so canvas stays pointer-events none)
    ...
    const loop = (t: number) => {
      ...
      if (mouseDampening > 0) {
        ...
        const dtSec = (t - lastTimeRef.current) / 1000;
        lastTimeRef.current = t;

        const tau = Math.max(1e-4, mouseDampening);
        let factor = 1 - Math.exp(-dtSec / tau);
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full overflow-hidden relative pointer-events-none touch-none select-none ${className}`}
      style={{
        ...style,
